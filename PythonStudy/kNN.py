...
def make_prediction_grid(predictors,outcomes,limits,h,k):
    """" Classsify each point on the prediction grid. """
    ...
    for i,x in enumerate(xs):
        for j,y in enumerate(ys):
            p = np.array([x,y])
            prediction_grid[j,i] = knn_predic(p,predictors,outcomes,k)
...
my_predictions = np.array([knn_predic(p,predictors,outcomes,5) for p in predictors])
